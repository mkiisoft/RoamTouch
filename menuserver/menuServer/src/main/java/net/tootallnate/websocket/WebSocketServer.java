...
public abstract class WebSocketServer extends WebSocketAdapter implements Runnable {
  ...
  public void sendToAll(String text) throws IOException {
	  ...
	  Log.v("socket","SendToAll: "+ text);    
	  
	  //The message keyboard opened 
	  /*if (text.contains("keyboardUp")){	  
		  send(text);
		  SwifteeApplication.setRemoteKeyboardOpened(true);
	  }*/
	  
	  send(text);   
	  
	  //if (!SwifteeApplication.getRemoteKeyboardOpened()) { // && !text.contains("net.tootallnate.websocket.WebSocket@")){
	  //  send(text);   
	  //}  
